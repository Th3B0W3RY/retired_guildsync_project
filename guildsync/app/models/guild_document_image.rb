...
class GuildDocumentImage < ApplicationRecord
  belongs_to :guild
  belongs_to :user
  ...
  # Public URL for the image (path only so it works without default_url_options[:host]).
  # Uses resized variant when libvips is available; otherwise raw blob path. Path is same-origin so img src works.
  def public_url
    ...
    Rails.logger.warn("GuildDocumentImage#public_url: variant failed (#{e.class}), using blob path: #{e.message}")
    safe_blob_path
  rescue StandardError => e
    Rails.logger.warn("GuildDocumentImage#public_url: #{e.class} - #{e.message}, using blob path")
    safe_blob_path
  end
  ...
  def safe_blob_path
    ...
    Rails.logger.warn("GuildDocumentImage#safe_blob_path: #{e.message}")
    nil
  end
