...
# Guild owner: search for a listed guild in an alliance and submit a join request (fixed requesting guild).
class GuildAllianceJoinRequestsController < ApplicationController
  ...
  def new
    preserve_session
    if @guild.alliance_guild&.active?
      redirect_to guild_path(@guild), alert: t("alliances.join_requests.errors.already_in_alliance")
      return
    end
  end
  ...
  def status
    preserve_session
    @outgoing = AllianceJoinRequest.where(requesting_guild_id: @guild.id, status: :pending).includes(:alliance).order(created_at: :desc)
  end
  ...
  def require_guild_owner!
    ...
    redirect_to dashboard_path, alert: t("alliances.join_requests.errors.unauthorized")
  end
